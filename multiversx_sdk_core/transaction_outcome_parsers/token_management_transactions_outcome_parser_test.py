...
def test_parse_issue_fungible():
    ...
    tx_results_and_logs = TransactionResultsAndLogsHolder([empty_result], tx_log)

    identifier = parser.parse_issue_fungible(tx_results_and_logs)
...
def test_parse_issue_non_fungible():
    ...
    identifier = parser.parse_issue_non_fungible(tx_results_and_logs)
...
def test_parse_issue_semi_fungible():
    ...
    identifier = parser.parse_issue_semi_fungible(tx_results_and_logs)
...
def test_parse_register_meta_esdt():
    ...
    identifier = parser.parse_register_meta_esdt(tx_results_and_logs)
...
def test_parse_register_and_set_all_roles():
    ...
    tx_results_and_logs = TransactionResultsAndLogsHolder([sc_result], tx_log)
    parsed = parser.parse_register_and_set_all_roles(tx_results_and_logs)

    assert parsed.token_identifier == "LMAO-d9f892"
    assert parsed.roles == ["ESDTRoleLocalMint", "ESDTRoleLocalBurn"]
...
def test_parse_set_special_role():
    ...
    parsed = parser.parse_set_special_role(tx_results_and_logs)
    assert parsed.user_address == "erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2"
    assert parsed.token_identifier == "METATEST-e05d11"
    assert parsed.roles == ["ESDTRoleNFTCreate", "ESDTRoleNFTAddQuantity", "ESDTRoleNFTBurn"]
...
def test_parse_nft_create():
    ...
    parsed = parser.parse_nft_create(tx_results_and_logs)
    assert parsed.token_identifier == "NFT-f01d1e"
    assert parsed.nonce == "1"
    assert parsed.initial_quantity == "1"
...
def test_parse_local_mint():
    ...
    parsed = parser.parse_local_mint(tx_results_and_logs)
    assert parsed.user_address == "erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2"
    assert parsed.token_identifier == "AAA-29c4c9"
    assert parsed.nonce == ""
    assert parsed.minted_supply == "100000"


def test_parse_local_burn():
    event = TransactionEvent(
        address="erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2",
    ...
    parsed = parser.parse_local_burn(tx_results_and_logs)
    assert parsed.user_address == "erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2"
    assert parsed.token_identifier == "AAA-29c4c9"
    assert parsed.nonce == ""
    assert parsed.burnt_supply == "100000"
...
def test_parse_pause():
    ...
    identifier = parser.parse_pause(tx_results_and_logs)
    assert identifier == "AAA-29c4c9"


def test_parse_unpause():
    event = TransactionEvent(
        address="erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2",
    ...
    identifier = parser.parse_unpause(tx_results_and_logs)
    assert identifier == "AAA-29c4c9"
...
def test_parse_freeze():
    ...
    tx_results_and_logs = TransactionResultsAndLogsHolder([sc_result], TransactionLogs())

    parsed = parser.parse_freeze(tx_results_and_logs)
    assert parsed.user_address == "erd1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssycr6th"
    assert parsed.token_identifier == "AAA-29c4c9"
    assert parsed.nonce == ""
    assert parsed.balance == "10000000"


def test_parse_unfreeze():
    event = TransactionEvent(
        address="erd1qqqqqqqqqqqqqqqpqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqzllls8a5w6u",
    ...
    parsed = parser.parse_unfreeze(tx_results_and_logs)
    assert parsed.user_address == "erd1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssycr6th"
    assert parsed.token_identifier == "AAA-29c4c9"
    assert parsed.nonce == ""
    assert parsed.balance == "10000000"
...
def test_parse_wipe():
    ...
    parsed = parser.parse_wipe(tx_results_and_logs)
    assert parsed.user_address == "erd1qyu5wthldzr8wx5c9ucg8kjagg0jfs53s8nr3zpz3hypefsdd8ssycr6th"
    assert parsed.token_identifier == "AAA-29c4c9"
    assert parsed.nonce == ""
    assert parsed.balance == "10000000"
...
def test_parse_update_attributes():
    ...
    tx_results_and_logs = TransactionResultsAndLogsHolder([tx_result], tx_log)

    parsed = parser.parse_update_attributes(tx_results_and_logs)
    assert parsed.token_identifier == "NFT-f01d1e"
    assert parsed.nonce == "1"
    assert parsed.attributes.decode() == "metadata:ipfsCID/test.json;tags:tag1,tag2"


def test_parse_add_quantity():
    event = TransactionEvent(
        address="erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2",
    ...
    parsed = parser.parse_add_quantity(tx_results_and_logs)
    assert parsed.token_identifier == "SEMIFNG-2c6d9f"
    assert parsed.nonce == "1"
    assert parsed.added_quantity == "10"


def test_parse_burn_quantity():
    event = TransactionEvent(
        address="erd18s6a06ktr2v6fgxv4ffhauxvptssnaqlds45qgsrucemlwc8rawq553rt2",
    ...
    parsed = parser.parse_burn_quantity(tx_results_and_logs)
    assert parsed.token_identifier == "SEMIFNG-2c6d9f"
    assert parsed.nonce == "1"
    assert parsed.burnt_quantity == "16"
